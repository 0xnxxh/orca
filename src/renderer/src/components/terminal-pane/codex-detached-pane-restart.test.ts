...
describe('codex detached pane restart executor', () => {
    ...
    ;(globalThis as { window: typeof window }).window = {
      ...
      api: {
        ...
        pty: {
          ...
          spawn: vi.fn().mockResolvedValue({ id: NEW_PTY, mutationAccess: { mode: 'legacy' } }),
  ...
  it('reaps a detached spawn and requeues when a pane mounts during the spawn', async () => {
    seedQueuedRestart()
    const pendingSpawn = deferred<Awaited<ReturnType<typeof window.api.pty.spawn>>>()
    ...
    try {
      pendingSpawn.resolve({ id: NEW_PTY, mutationAccess: { mode: 'legacy' } })
