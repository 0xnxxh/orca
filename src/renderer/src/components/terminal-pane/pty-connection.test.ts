...
describe('connectPanePty', () => {
  ...
  it('refits but does not report the destination grid from a background measure-lease pane', async () => {
    ...
    // A measure-lease pane is fully sized, so the fit lands — but reporting from a
    // hidden pane would persist as a claim on the next subscribe. Its grid stays
    // unreported until the next layout change after reveal; see the send condition.
    expect(pane.fitAddon.fit).toHaveBeenCalled()
    expect(pane.terminal.cols).toBe(120)
    expect(transport.resize).not.toHaveBeenCalled()
  })
