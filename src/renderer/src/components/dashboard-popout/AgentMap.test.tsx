...
function renderRings(
  cards: DashboardCard[],
  {
    ...
    onSpawnAgent?: React.ComponentProps<typeof AgentMap>['onSpawnAgent']
    launchableAgentsByWorktreeId?: React.ComponentProps<
      typeof AgentMap
    >['launchableAgentsByWorktreeId']
...
describe('AgentMap', () => {
  ...
  it('shows worktree details and starts an agent in that workspace', () => {
    const onSpawnAgent = vi.fn()
    const running = card()
    const { container } = renderRings([running], {
      onSpawnAgent,
      launchableAgentsByWorktreeId: { 'worktree-1': ['codex'] }
    })

    fireEvent.click(container.querySelector('.agent-map-worktree-label')!)
    expect(screen.queryByText('Start a new agent')).not.toBeInTheDocument()
