...
type FleetProjectOption = {
  id: string
  name: string
  count: number
}

type FleetRingsFocusRailProps = {
  counts: FleetFocusCounts
  ...
  totalCount: number
  visibleCount: number
  onFinishedScopeChange: (scope: FleetFinishedScope) => void
  onFit: () => void
  onMarkReviewed: (cards: DashboardCard[]) => void
  onProjectToggle: (projectId: string) => void
  onShowAll: () => void
  onStateToggle: (state: FleetFocusState) => void
}

const STATE_ROWS: {
  state: FleetFocusState
...
const FINISHED_SCOPES: FleetFinishedScope[] = ['review', 'day', 'week', 'all']

function stateLabel(state: FleetFocusState): string {
  switch (state) {
    case 'attention':
    ...
    case 'finished':
      return translate('dashboardPopout.rings.focus.finished', 'Finished')
  }
}

function finishedScopeLabel(scope: FleetFinishedScope): string {
  switch (scope) {
    case 'review':
      return translate('dashboardPopout.rings.focus.toReview', 'To review')
    case 'day':
      return translate('dashboardPopout.rings.focus.lastDay', 'Last 24 hours')
    case 'week':
      return translate('dashboardPopout.rings.focus.lastWeek', 'Last 7 days')
    case 'all':
      return translate('dashboardPopout.rings.focus.allFinished', 'All finished')
  }
}

export function FleetRingsFocusRail({
  counts,
  enabledStates,
  finishedScope,
  hiddenProjectIds,
  ...
  onShowAll,
  onStateToggle
}: FleetRingsFocusRailProps): React.JSX.Element {
  return (
    <aside className="scrollbar-sleek hidden min-h-0 w-56 shrink-0 overflow-y-auto border-r border-border bg-card/35 p-3 lg:flex lg:flex-col">
      <header className="border-b border-border px-1 pb-3">
        <span className="text-[11px] font-semibold uppercase tracking-[0.05em] text-muted-foreground">
          {translate('dashboardPopout.rings.focus.title', 'Focus view')}
        </span>
        <strong className="mt-1 block text-xs">
          {translate('dashboardPopout.rings.focus.control', 'You decide what stays visible')}
        </strong>
        <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">
          {translate(
            'dashboardPopout.rings.focus.reviewCopy',
      ...
      <section className="border-b border-border py-3">
        <span className="px-1 text-[11px] font-semibold uppercase tracking-[0.05em] text-muted-foreground">
          {translate('dashboardPopout.rings.focus.showStates', 'Show agent states')}
        </span>
      ...
      <section className="border-b border-border py-3">
        <span className="px-1 text-[11px] font-semibold uppercase tracking-[0.05em] text-muted-foreground">
          {translate('dashboardPopout.rings.focus.finishedResults', 'Finished results')}
        </span>
        <div className="mt-1.5 space-y-0.5">
        ...
        <p className="mt-2 flex items-center gap-1.5 px-1 text-[10px] text-muted-foreground">
          ...
          {translate('dashboardPopout.rings.focus.pinnedVisible', 'Pinned results stay visible.')}
      ...
      <div className="border-b border-border px-1 py-3">
        ...
        <span className="text-[10px] text-muted-foreground">
          {translate('dashboardPopout.rings.focus.hidden', '{{count}} agents hidden', {
      ...
      >
        ...
        {translate('dashboardPopout.rings.focus.reviewVisible', 'Mark visible results reviewed')}
      </Button>

      <div className="mt-auto grid grid-cols-2 gap-1 pt-3">
        <Button type="button" variant="ghost" size="xs" onClick={onShowAll}>
          {translate('dashboardPopout.rings.focus.showAll', 'Show all')}
        </Button>
        <Button type="button" variant="outline" size="xs" onClick={onFit}>
          <Focus className="size-3" />
          {translate('dashboardPopout.rings.fit', 'Fit')}
        </Button>
      </div>
    </aside>
  )
