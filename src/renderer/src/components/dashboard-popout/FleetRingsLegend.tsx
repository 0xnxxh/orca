...
import type { FleetProjectRing, FleetRingsLayout } from './fleet-rings-layout'

type FleetRingsLegendProps = {
  layout: FleetRingsLayout
  onFocusProject: (project: FleetProjectRing) => void
  onFit: () => void
}
...
function stateLabel(state: DashboardCardDotState): string {
  switch (state) {
    case 'waiting':
      return translate('dashboardPopout.rings.state.waiting', 'Waiting for input')
    case 'blocked':
      return translate('dashboardPopout.rings.state.blocked', 'Blocked')
    case 'working':
      return translate('dashboardPopout.rings.state.working', 'Working')
    case 'done':
      return translate('dashboardPopout.rings.state.done', 'Done')
    case 'idle':
      return translate('dashboardPopout.rings.state.idle', 'Idle')
  }
}

export function FleetRingsLegend({
  layout,
  onFocusProject,
  onFit
}: FleetRingsLegendProps): React.JSX.Element {
    ...
    <aside className="scrollbar-sleek hidden min-h-0 w-52 shrink-0 overflow-y-auto border-r border-border bg-card/35 p-3 lg:flex lg:flex-col">
      <header className="border-b border-border px-1 pb-3">
        <span className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.05em] text-muted-foreground">
          ...
          {translate('dashboardPopout.rings.canvasLayers', 'Canvas layers')}
        </span>
        <strong className="mt-1 block text-xs">
          {translate('dashboardPopout.rings.hierarchy', 'Project → workspace → agent')}
        </strong>
        <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">
          {translate(
            'dashboardPopout.rings.hierarchyCopy',
            ...
            >
              <span className="min-w-0">
                ...
                <span className="block text-[10px] text-muted-foreground">
                  {translate('dashboardPopout.rings.workspaceCount', '{{count}} workspaces', {
      ...
      <section className="border-b border-border py-3">
        <span className="px-1 text-[11px] font-semibold uppercase tracking-[0.05em] text-muted-foreground">
          {translate('dashboardPopout.rings.agentState', 'Agent state')}
        </span>
        <div className="mt-2 space-y-2">
          {STATUS_ROWS.map((state) => (
            ...
            >
              <span className={`fleet-legend-dot fleet-status-${state}`} aria-hidden />
      ...
      <section className="border-b border-border py-3">
        <span className="px-1 text-[11px] font-semibold uppercase tracking-[0.05em] text-muted-foreground">
          {translate('dashboardPopout.rings.agentSize', 'Agent size')}
        </span>
        ...
        <div className="mt-1 flex justify-between text-[10px] text-muted-foreground">
          <span>{translate('dashboardPopout.rings.justStarted', 'Just started')}</span>
          <span>{translate('dashboardPopout.rings.workingLonger', 'Working longer')}</span>
        </div>
        <p className="mt-1 text-[10px] text-muted-foreground">
          {translate('dashboardPopout.rings.sizeScale', 'Logarithmic scale · capped after 60m')}
        </p>
      </section>

      <section className="space-y-2 px-1 py-3 text-[10px]">
        <p className="flex justify-between">
          <strong>{translate('dashboardPopout.rings.hover', 'Hover')}</strong>
          <span className="text-muted-foreground">
            {translate('dashboardPopout.rings.previewTerminal', 'Preview terminal')}
          </span>
        </p>
        <p className="flex justify-between">
          <strong>{translate('dashboardPopout.rings.click', 'Click')}</strong>
          <span className="text-muted-foreground">
            {translate('dashboardPopout.rings.pinAgent', 'Pin agent')}
          </span>
        </p>
        <p className="flex justify-between">
          <strong>{translate('dashboardPopout.rings.doubleClick', 'Double-click')}</strong>
          <span className="text-muted-foreground">
            {translate('dashboardPopout.rings.zoomWorkspace', 'Zoom workspace')}
          </span>
        </p>
      </section>
      ...
      <Button type="button" variant="outline" size="xs" className="mt-auto" onClick={onFit}>
        ...
        {translate('dashboardPopout.rings.fitFleet', 'Fit fleet')}
      </Button>
    </aside>
  )
}
