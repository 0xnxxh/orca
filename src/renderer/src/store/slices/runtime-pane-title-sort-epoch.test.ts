...
describe('runtimePaneTitle → sortEpoch', () => {
  ...
  // Why: the tab-owner cache is last-writer-wins, so a tab id held under two
  // worktrees names a background one for a pane that is in the active worktree
  // — and every classified title frame then re-ranks the sidebar.
  it('does not bump sortEpoch when a duplicated tab id is also in the active worktree', () => {
    const store = createTestStore()
    seedStore(store, {
      worktreesByRepo: {
        repo1: [
      ...
      tabsByWorktree: {
        'wt-a': [makeTab({ id: 'tab-1', worktreeId: 'wt-a' })],
        'wt-b': [makeTab({ id: 'tab-1', worktreeId: 'wt-b' })]
      },
      activeWorktreeId: 'wt-a'
    })
    const baseline = store.getState().sortEpoch
  ...
  it('does not bump sortEpoch clearing a duplicated tab id held by the active worktree', () => {
    const store = createTestStore()
    seedStore(store, {
      worktreesByRepo: {
        repo1: [
  ...
  // Why: the duplicate must not flip the gate the other way either — a pane in
  // no active worktree still has to re-rank.
  it('still bumps sortEpoch when neither owner of a duplicated tab id is active', () => {
    const store = createTestStore()
    seedStore(store, {
      worktreesByRepo: {
        repo1: [
          makeWorktree({ id: 'wt-a', repoId: 'repo1', path: '/path/wt-a' }),
          makeWorktree({ id: 'wt-b', repoId: 'repo1', path: '/path/wt-b' }),
          makeWorktree({ id: 'wt-c', repoId: 'repo1', path: '/path/wt-c' })
        ]
      },
      tabsByWorktree: {
        'wt-a': [makeTab({ id: 'tab-1', worktreeId: 'wt-a' })],
        'wt-b': [makeTab({ id: 'tab-1', worktreeId: 'wt-b' })],
        'wt-c': [makeTab({ id: 'tab-2', worktreeId: 'wt-c' })]
      },
