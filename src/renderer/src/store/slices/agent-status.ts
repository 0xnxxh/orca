...
export type AgentStatusSlice = {
  ...
  /** Apply ordered status updates in one store publication. */
  setAgentStatuses: (updates: readonly AgentStatusBatchUpdate[]) => boolean[]

  /** Fold caller-derived updates against exact staged state in one store publication. */
...
export const createAgentStatusSlice: StateCreator<AppState, [], [], AgentStatusSlice> = (
  ...
  const set = ((update: AgentStatusStateUpdate, replace?: boolean): void => {
    if (batchedAgentStatusState === null) {
      if (replace === true) {
        storeSet(update as AppState | ((state: AppState) => AppState), true)
      } else {
    ...
    batchedAgentStatusState =
      replace === true
        ? (nextState as AppState)
        : Object.assign({}, batchedAgentStatusState, nextState)
  }) as typeof storeSet
        ...
        if (
          ...
          history = [
            ...history,
            {
              ...
              ...(existing.state === 'done' && existing.lastAssistantMessage
