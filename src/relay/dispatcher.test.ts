...
import { RelayDispatcher, type ResponseSettlement, type SinkWriteSettlement } from './dispatcher'
import {
  encodeJsonRpcFrame,
...
describe('RelayDispatcher', () => {
  ...
  it('marks a delivered application error separately from a transport failure', async () => {
    let settlement: ResponseSettlement | undefined
    dispatcher.onRequest('fail.with-settlement', async (_params, context) => {
      context.onResponseSettled?.((result) => {
        settlement = result
      })
      throw new Error('application rejection')
    })

    dispatcher.feed(
      encodeJsonRpcFrame({ jsonrpc: '2.0', id: 6, method: 'fail.with-settlement' }, 1, 0)
    )
    await vi.advanceTimersByTimeAsync(0)

    expect(settlement).toBeDefined()
    expect(settlement).toMatchObject({ ok: false, responseDelivered: true })
  })
