...
/** Strips EVERY trailing numeric suffix, not just one. Once the pool is spent the suggester emits
 *  `nautilus-2`, and a collision on that yields `nautilus-2-3` — which a single strip leaves as
 *  `nautilus-2`, not a pool name, so retirement would silently no-op on exactly the tier where the
 *  pool is already exhausted. Deliberately not shared with `isAutoGeneratedCreatureBranchName`:
 *  auto-rename eligibility is a different question and its single-strip behavior is intended. */
