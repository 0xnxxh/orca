...
import { localTranscriptFileSource, type TranscriptFileSource } from './transcript-file-source'
import {
...
export async function readIncrementalTranscriptMessages(
  ...
  options: { maxDrainRetainedBytes?: number; fileSource?: TranscriptFileSource } = {}
): Promise<NativeChatMessage[]> {
  const fileSource = options.fileSource ?? localTranscriptFileSource
  const end = (await fileSource.stat(filePath)).size
  if (end <= state.offset) {
    return []
  }
  ...
  try {
    ...
    streamChunks: while (absoluteOffset < end) {
      const chunk = await reader.read(
        absoluteOffset,
        Math.min(INCREMENTAL_READ_CHUNK_BYTES, end - absoluteOffset)
      )
      if (chunk.length === 0) {
