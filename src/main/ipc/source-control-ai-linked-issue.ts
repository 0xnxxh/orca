 ...
 * (and `repoId`) before the meta read. A stale id after a workspace switch would
 * otherwise pull another workspace's issue number into this commit message.
...
export function resolveSourceControlAiLinkedIssue(
  ...
  if (args.repoId && parsed.repoId !== args.repoId) {
    return null
  }
  ...
  return typeof linkedIssue === 'number' && Number.isFinite(linkedIssue) ? linkedIssue : null
}
