...
// Why: a CLI-shaped launch must never reach Chromium startup — it aborts on a
// headless or unsandboxed Linux host — and on Windows a launch that lost
// ELECTRON_RUN_AS_NODE would boot the GUI and exit silently. Run before the lock
// gate below, and before the serve-argv rewrite: rewriting first replaces the
// `serve` positional, so the command-name lookup would find a port number and
// strand the launch in an in-process serve.
