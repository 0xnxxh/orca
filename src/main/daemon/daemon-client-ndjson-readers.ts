...
import { settlePendingResponse, type PendingRequest } from './daemon-client-pending-requests'
...
export function attachControlResponseReader(
  ...
  const parser = createNdjsonParser(
    (msg) => {
      settlePendingResponse(pendingRequests, msg as RpcResponse)
    },
...
export function attachStreamEventReader(
  socket: Socket,
  eventListeners: ((event: unknown) => void)[]
): () => void {
