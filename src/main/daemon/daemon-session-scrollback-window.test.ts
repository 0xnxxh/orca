...
describe('resolveDaemonSessionScrollbackRows', () => {
  ...
  it('accepts a bounded override and rejects everything outside it', () => {
    const ok = { ORCA_DAEMON_SESSION_SCROLLBACK_ROWS: '2500' } as NodeJS.ProcessEnv
    expect(resolveDaemonSessionScrollbackRows(ok)).toBe(2500)
    ...
    for (const raw of ['0', '50', '50000', '-1', '3.5', 'nonsense', '']) {
