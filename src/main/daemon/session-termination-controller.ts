...
export type SessionTerminationOptions = {
  sessionId: string
...
/** Termination escalation for one session: graceful kill (with the agent-only descendant sweep),
 *  the force-kill deadline and its bounded retry, the physical-exit await, and the once-only
 *  native-handle release that frees the ptmx fd. */
export class SessionTerminationController {
  ...
  constructor(private readonly opts: SessionTerminationOptions) {
      ...
      void Promise.resolve(
        killWithDescendantSweep(
          ...
          },
          {
            ...
            ownsRoot: () => this.opts.getState() !== 'exited'
          }
        )
      ).catch((error) => {
        if (this.opts.getState() !== 'exited') {
  ...
  private requestForceKill(): void {
    if (this.opts.getState() === 'exited' || this.forceKillSent) {
      return
    }
      ...
      if (attempt + 1 < SESSION_FORCE_KILL_MAX_ATTEMPTS) {
        try {
          await this.physicalExit.waitForExit(
            ...
            () => new Error(`Retrying force-kill for PTY ${this.opts.sessionId}`)
          )
          return
        } catch {
  ...
  private waitForPhysicalExit(timeoutMs: number): Promise<void> {
    ...
    return this.physicalExit.waitForExit(
      timeoutMs,
      () => new Error(`Timed out waiting for PTY process exit: ${this.opts.sessionId}`)
    )
  }
}
