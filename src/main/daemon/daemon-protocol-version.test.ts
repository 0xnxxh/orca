...
import {
  ...
  PROTOCOL_VERSION,
  PTY_STREAM_BINDING_DAEMON_PROTOCOL_VERSION,
  supportsExactPtyOperations,
...
describe('daemon protocol version', () => {
  it('ships bounded history transfer after the 2031-unsubscribe fact', () => {
    ...
    expect(EXACT_HELD_PRODUCER_PAUSE_DAEMON_PROTOCOL_VERSION).toBe(35)
    expect(TERMINAL_SESSION_AUTHORITY_DAEMON_PROTOCOL_VERSION).toBe(34)
    expect(EXACT_PTY_OPERATIONS_DAEMON_PROTOCOL_VERSION).toBe(33)
    expect(PTY_STREAM_BINDING_DAEMON_PROTOCOL_VERSION).toBe(33)
  ...
  it('admits exact held pauses only at the acknowledged v35 boundary', () => {
    expect(supportsExactHeldProducerPause(34)).toBe(false)
    expect(supportsExactHeldProducerPause(35)).toBe(true)
  })

  it('keeps v33 incarnation fencing explicit but authority-unavailable', () => {
    expect(supportsExactPtyOperations(33)).toBe(true)
    expect(supportsTerminalSessionAuthority(33)).toBe(false)
    expect(supportsTerminalSessionAuthority(34)).toBe(true)
  })

  it('advertises the complete exact contract at one coherent v33 boundary', () => {
    expect(supportsExactPtyOperations(32)).toBe(false)
    expect(supportsExactPtyOperations(33)).toBe(true)
    expect(supportsPtyStreamBinding(32)).toBe(false)
    expect(supportsPtyStreamBinding(33)).toBe(true)
  })
