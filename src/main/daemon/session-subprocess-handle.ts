/** The PTY handle contract `pty-subprocess.ts` implements and `Session` (plus the daemon
 *  server/adapter layers) consumes. Lives apart from `session.ts` so the implementer does not
 *  have to import its own contract from a consumer. */
