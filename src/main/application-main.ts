/* eslint-disable max-lines -- Why: this is Orca's main-process application module;
...
import {
  ...
  isQuittingForUpdate,
  reportPreviousUpdateInstallFailure
} from './updater'
...
app.whenReady().then(async () => {
  ...
  if (process.platform === 'darwin' && app.isPackaged) {
    // Why: the gate and detached monitor cannot initialize userData or normal
    // telemetry, so the first successful production startup forwards them.
    const { failedInstall } = ingestMacUpdateFenceDiagnostics()
    // Why: running the target version means the install landed after all;
    // only a still-not-updated app owes the user a failure notice.
    if (failedInstall && failedInstall.targetVersion !== app.getVersion()) {
      reportPreviousUpdateInstallFailure(failedInstall.targetVersion)
    }
  }
