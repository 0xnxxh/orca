...
function annotateTypingMeasurement(
  ...
  const rendererSkipSummary = debug
    ? ` rendererSkips=${debug.hiddenRendererSkipCount} rendererSkippedChars=${debug.hiddenRendererSkippedChars}`
    : ''
  ...
  testInfo.annotations.push({
    ...
    description: `panes=${paneCount} frames=${measurement.frameCount} median=${measurement.medianLatencyMs.toFixed(
      ...
      .join(',')}${rendererSkipSummary}${schedulerSummary}${mainPressureSummary}${ackGateSummary}`
  })
}
