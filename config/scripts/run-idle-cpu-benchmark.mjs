...
import {
  DEFAULT_AGENT_STATUS_BATCH_INTERVAL_MS,
  DEFAULT_AGENT_STATUS_UPDATES_PER_BATCH,
  runAgentStatusBatchWorkload
} from './idle-cpu-agent-status-batch-workload.mjs'
import {
  descendantsOf,
  readProcessRows,
...
function parseArgs(argv) {
  const options = {
    ...
    lineageDepth: 0,
    agentsPerWorktree: 0,
    agentStatusBatches: 0,
    agentStatusUpdatesPerBatch: DEFAULT_AGENT_STATUS_UPDATES_PER_BATCH,
    agentStatusBatchIntervalMs: DEFAULT_AGENT_STATUS_BATCH_INTERVAL_MS,
    agentStatusWriteMode: 'batch',
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (arg === '--agent-status-batches') {
      options.agentStatusBatches = Number(readValue())
    } else if (arg === '--agent-status-updates-per-batch') {
      options.agentStatusUpdatesPerBatch = Number(readValue())
    } else if (arg === '--agent-status-batch-interval-ms') {
      options.agentStatusBatchIntervalMs = Number(readValue())
    } else if (arg === '--agent-status-write-mode') {
      options.agentStatusWriteMode = readValue()
  ...
  for (const key of [
    ...
    'agentsPerWorktree',
    'agentStatusBatches',
    'agentStatusUpdatesPerBatch',
    'agentStatusBatchIntervalMs',
    'zustandPublications',
  ...
  options.agentStatusBatches = Math.floor(options.agentStatusBatches)
  options.agentStatusUpdatesPerBatch = Math.floor(options.agentStatusUpdatesPerBatch)
  options.agentStatusBatchIntervalMs = Math.max(1, Math.floor(options.agentStatusBatchIntervalMs))
  ...
  if (!['batch', 'sequential'].includes(options.agentStatusWriteMode)) {
    throw new Error(`Invalid --agent-status-write-mode: ${options.agentStatusWriteMode}`)
  }
  if (options.lineageDepth > 0 && options.worktrees < 2) {
    throw new Error('--lineage-depth requires at least two --worktrees')
  }
  if (options.agentStatusBatches > 0 && options.agentStatusUpdatesPerBatch === 0) {
    throw new Error('--agent-status-updates-per-batch must be positive when batches are requested')
  }
  if (options.agentStatusBatches > 0 && options.agentsPerWorktree === 0) {
    throw new Error('--agent-status-batches requires --agents-per-worktree')
  }
  if (options.agentStatusBatches > 0 && options.zustandPublications > 0) {
    throw new Error('Agent-status batches and no-op Zustand publications are separate workloads')
  }
  const requestedAgentStatusUpdates =
    options.agentStatusBatches * options.agentStatusUpdatesPerBatch
  if (!Number.isSafeInteger(requestedAgentStatusUpdates)) {
    throw new Error(
      `Agent-status update count is not a safe integer: ${requestedAgentStatusUpdates}`
    )
  }
  ...
  const agentStatusSpanMs =
    Math.max(0, options.agentStatusBatches - 1) * options.agentStatusBatchIntervalMs
  if (agentStatusSpanMs > options.sampleMs) {
    throw new Error(
      `Agent-status batch span ${agentStatusSpanMs}ms exceeds --sample-ms ${options.sampleMs}`
    )
  }
  return options
}
...
async function main() {
  ...
  try {
    ...
    console.log(
      `[idle-cpu] root pid=${rootPid}; warmup=${options.warmupMs}ms sample=${options.sampleMs}ms interval=${options.intervalMs}ms worktrees=${options.worktrees} lineage-depth=${options.lineageDepth} agents/worktree=${options.agentsPerWorktree} status-batches=${options.agentStatusBatches} status-updates/batch=${options.agentStatusUpdatesPerBatch} status-mode=${options.agentStatusWriteMode} publications=${options.zustandPublications}`
    )
    ...
    const agentStatusWorkloadPromise = runAgentStatusBatchWorkload(page, {
      batchCount: options.agentStatusBatches,
      updatesPerBatch: options.agentStatusUpdatesPerBatch,
      intervalMs: options.agentStatusBatchIntervalMs,
      writeMode: options.agentStatusWriteMode
    })
    const sampled = await sampleProcessTreeUntilWorkloadsComplete({
      rootPid,
      requestedDurationMs: options.sampleMs,
      intervalMs: options.intervalMs,
      workloadPromise: Promise.all([publicationPromise, agentStatusWorkloadPromise])
    })
    const samples = sampled.samples
    const [zustandPublications, agentStatusWorkload] = sampled.workloadResult
      ...
      JSON.stringify(
        {
          ...
          agentStatusWorkload: report.agentStatusWorkload,
