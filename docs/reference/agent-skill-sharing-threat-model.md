...
Private means access-controlled to authenticated Orca users and organizations. V1 is not
...
| TM-09 | IDOR or stale organization membership exposes package metadata or creates a fresh grant.                                                          | Every resolution and grant request authenticates and re-evaluates current package/share ACLs. Membership removal is tested between preview, grant, and download. Not-found responses hide unauthorized existence.                                                  | Human authorization review and organization-departure policy approval remain required.              |
| TM-10 | Content-addressed deduplication discloses another tenant's package through response shape or timing.                                              | Tenant-scoped APIs never expose GCS keys, generations, or whether an object already existed; existing-object reuse verifies both archive and logical package identity.                                                                                             | A staging timing test and reviewed mitigation threshold are still required before external rollout. |
...
| TM-15 | Telemetry, logs, or support bundles leak instructions, filenames, paths, ACLs, grants, policies, or credentials.                                  | Desktop install diagnostics map values to bounded categories before tracing. Cloud request logs contain route, method, status, and duration only. Support-bundle regression tests exclude raw private operation data.                                              | Verify deployed staging logs and support uploads before enabling flags.                             |
| TM-16 | Permissive staging permissions expose package bytes to another local user.                                                                        | Package archives, downloads, extraction, relayed uploads, locks, journals, and receipts use owner-private modes on POSIX; Windows uses owner-profile paths and inherited ACLs. Existing POSIX download roots are tightened before use.                             | Verify Windows ACLs, Linux floor behavior, WSL ownership, and SSH staging on real hosts.            |
