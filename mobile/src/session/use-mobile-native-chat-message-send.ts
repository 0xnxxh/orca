  ...
  const sendMessage = useCallback(
    async (
      ...
      const outcome = await sendMobileNativeChatMessageWithOutcome({
        ...
        terminal: handle,
        text,
        // Why: desktop-parity pre-clear. The stale-input heal above only fires for
        // terminals a mobile image paste marked, so a desktop launch-draft prefill
        // sitting on the input line would otherwise glue onto this message.
